...
{\
	int level = 0; \
	HTREEITEM mItem = _item; \
	while (mItem!=0 && mItem!=TVI_ROOT) { \
		level++; \
		mItem = m_pTree->GetParentItem(mItem); \
	} \
	FILE *fp = fopen("tree", "a+t"); \
	fprintf(fp, "%-2d %s\r\n", level, m_outputInfo); \
	fclose(fp); \
}
#endif
...
void CNalParser::h264_debug_slice_header(h264_stream_t* h, HTREEITEM root)
{
    ...
    else
    {
        ...
        if( ! is_slice_type( sh->slice_type, SH_SLICE_TYPE_I ) && ! is_slice_type( sh->slice_type, SH_SLICE_TYPE_SI ) )
        {
            ...
            {
                for (unsigned int i = 0; i < sh->rplm.size; i++)
                {
    ...
    if( nal->nal_ref_idc != 0 )
    {
        ...
        else
        {
            ...
            {
                for (unsigned int i = 0; i < sh->drpm.size; i++)
                {
