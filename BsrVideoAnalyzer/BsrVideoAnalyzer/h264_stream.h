...
typedef struct
{
	...
	int *slice_group_id; // FIXME what size?
#endif
...
inline void free_SliceHdr(slice_header_t *sh)
{
	if (sh)
	{
		free(sh);
...
inline void init_SliceHdr(slice_header_t *sh)
{
	if (sh)
	{
	...
	//if (sh)
	//{
	//	memset(sh, 0, (SLICE_S1 + SLICE_S2));
	//	sh->rplm.ref_pic_list_modification_flag_l0 = 0;
	//	sh->rplm.ref_pic_list_modification_flag_l1 = 0;
	//	sh->rplm.rplm.clear();

	//	sh->drpm.adaptive_ref_pic_marking_mode_flag = 0;
	//	sh->drpm.long_term_reference_flag = 0;
	//	sh->drpm.no_output_of_prior_pics_flag = 0;
	//	sh->drpm.drpm.clear();
	//}
}
