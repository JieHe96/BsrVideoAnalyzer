...
CHexEdit::CHexEdit()
{
	...
	HRSRC hRsrc = FindResource(NULL, MAKEINTRESOURCE(IDR_FONT_HEX), RT_FONT);// TEXT("Font"));
	DWORD cbSize = SizeofResource(hInstRes, hRsrc);
	hMem = LoadResource(hInstRes, hRsrc);
	LPVOID pvData = LockResource(hMem);
	DWORD nFontsInstalled = 0;
	hFontInstalled = AddFontMemResourceEx(pvData, cbSize, NULL, &nFontsInstalled);
	
	m_Font.CreateFont(-11, 0, 0, 0, FW_NORMAL, 0, 0, 0, 0, 0, 0, DRAFT_QUALITY, FIXED_PITCH, "mononoki");// "Courier New");
...
void CHexEdit::OnPaint() 
{
	...
	if(m_pData)
	{
		...
		if(m_bShowAddress)
		{
			char fmtx[8] = { '0','x','%','0','8','l','X','\0' };
			char fmtd[8] = { '%','8','l','d',' ','\0' };
			char *fmt;
			if (m_bAddrHex)
			{
				fmt = fmtx;
				fmt[4] = m_bAddressIsWide ? '6' : '4';
			}
			else
			{
				fmt = fmtd;
				fmt[1] = m_bAddressIsWide ? '7' : '4';
			}
...
{
	if(bpr==1 || bpr==2 || bpr==4 || bpr==8 || bpr == 16 || bpr == 24 || bpr == 32)
		m_bpr = bpr;
	else
		return;
	m_bUpdate= TRUE;
...
void CHexEdit::UpdateScrollbars()
{
	...
	si.fMask = SIF_ALL;
	si.nMin = 0;
	si.nMax = (m_length+(m_bpr-1)) / m_bpr - 1;
	si.nPage = m_lpp;
