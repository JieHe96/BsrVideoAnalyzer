...
h264_stream_t* h264_new()
{
	...
	for (int i = 0; i < 256; i++) { h->pps_table[i] = (pps_t*)calloc(1, sizeof(pps_t)); h->pps_table[i]->slice_group_id = NULL; }
...
void h264_free(h264_stream_t* h)
{
    ...
    for ( int i = 0; i < 256; i++ ) 
	{ 
		if (h->pps_table[i]->slice_group_id)
			free(h->pps_table[i]->slice_group_id);
		free( h->pps_table[i] ); 
	}
	...
	if (h->slice_data && h->slice_data->rbsp_buf) free(h->slice_data->rbsp_buf);
...
int read_nal_unit(h264_stream_t* h, uint8_t* buf, int size)
{
    ...
    if (rc < 0) { free(rbsp_buf); return rc; } // handle conversion error
...
void read_pic_parameter_set_rbsp(h264_stream_t* h, bs_t* b)
{
	...
	if (h->pps_table[pps_id]->slice_group_id)
	{
		free(h->pps_table[pps_id]->slice_group_id);
		h->pps_table[pps_id]->slice_group_id = NULL;
	}
    ...
    if( pps->num_slice_groups_minus1 > 0 )
    {
        ...
        else if( pps->slice_group_map_type == 6 )
        {
            ...
            pps->slice_group_id = (int *)malloc((pps->pic_size_in_map_units_minus1 + 1)*sizeof(int));
    ...
    if( ! is_slice_type( sh->slice_type, SH_SLICE_TYPE_I ) && ! is_slice_type( sh->slice_type, SH_SLICE_TYPE_SI ) )
    {
        ...
        if( sh->rplm.ref_pic_list_modification_flag_l0 )
        {
            do
            {
				...
				memcpy(&(sh->rplm.rplm[sh->rplm.size]), &rplmtt, sizeof(rplm_tt));
				sh->rplm.size++;
    ...
    else
    {
        ...
        if( sh->drpm.adaptive_ref_pic_marking_mode_flag )
        {
            do
            {
				...
				memcpy(&(sh->drpm.drpm[sh->drpm.size]), &drpmtt, sizeof(drpm_tt));
				sh->drpm.size++;
