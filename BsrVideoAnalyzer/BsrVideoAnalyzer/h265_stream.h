...
#define VPS_MAX_SUB_LAYERS_MINUS1	7

extern int rbsp_to_nal(const uint8_t* rbsp_buf, const int* rbsp_size, uint8_t* nal_buf, int* nal_size);
extern int nal_to_rbsp(const int nal_header_size, const uint8_t* nal_buf, int* nal_size, uint8_t* rbsp_buf, int* rbsp_size);
...
typedef struct
{
	...
	uint8_t sub_layer_profile_present_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_level_present_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	//*/
...
#else
	uint8_t sub_layer_profile_space[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_tier_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_profile_idc[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_profile_compatibility_flag[VPS_MAX_SUB_LAYERS_MINUS1][32];
	uint8_t sub_layer_progressive_source_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_interlaced_source_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_non_packed_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_frame_only_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_max_12bit_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_max_10bit_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_max_8bit_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_max_422chroma_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_max_420chroma_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_max_monochrome_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_intra_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_one_picture_only_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_lower_bit_rate_constraint_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint64_t sub_layer_reserved_zero_34bits[VPS_MAX_SUB_LAYERS_MINUS1];
	uint64_t sub_layer_reserved_zero_43bits[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_inbld_flag[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_reserved_zero_bit[VPS_MAX_SUB_LAYERS_MINUS1];
	uint8_t sub_layer_level_idc[VPS_MAX_SUB_LAYERS_MINUS1];
#endif
	//*/
} profile_tier_level_t;

inline void init_ptl_t(profile_tier_level_t *t)
{
	memset(t, 0, sizeof(profile_tier_level_t));
}
...
inline void init_subhrd_t(sub_layer_hrd_parameters_t *t)
{
	t->bit_rate_du_value_minus1.clear();
	t->bit_rate_value_minus1.clear();
	t->cpb_size_du_value_minus1.clear();
	t->cpb_size_value_minus1.clear();
	t->cbr_flag.clear();
}
...
#define PART_INIT(t, from, to) \
	memset(&(t->from), 0, (size_t)&(t->to) - (size_t)&(t->from) + sizeof(t->to))


inline void init_hrd_t(hrd_parameters_t *t)
{
//	memset(t, 0,(size_t)&(t->fixed_pic_rate_general_flag) - (size_t)&(t->nal_hrd_parameters_present_flag));
	PART_INIT(t, nal_hrd_parameters_present_flag, dpb_output_delay_length_minus1);
	t->fixed_pic_rate_general_flag.clear();
	t->fixed_pic_rate_within_cvs_flag.clear();
	t->elemental_duration_in_tc_minus1.clear();
	t->cpb_cnt_minus1.clear();
	init_subhrd_t(&t->sub_layer_hrd_parameters);
	init_subhrd_t(&t->sub_layer_hrd_parameters_v);
}
...
inline void init_pps_ex_t(pps_range_extension_t *t)
{
	t->log2_max_transform_skip_block_size_minus2 = 0;
	t->cross_component_prediction_enabled_flag = 0;
	t->chroma_qp_offset_list_enabled_flag = 0;
	t->diff_cu_chroma_qp_offset_depth = 0;
	t->chroma_qp_offset_list_len_minus1 = 0;
	t->cb_qp_offset_list.clear();
	t->cr_qp_offset_list.clear();
	t->log2_sao_offset_scale_chroma = 0;
	t->log2_sao_offset_scale_luma = 0;
}
...
inline void init_vui_t(vui_parameters_t *t)
{
	PART_INIT(t, aspect_ratio_info_present_flag, vui_hrd_parameters_present_flag);
	init_hrd_t(&t->hrd_parameters);
	PART_INIT(t, bitstream_restriction_flag, log2_max_mv_length_vertical);
}
...
inline void init_pred_weight_table_t(pred_weight_table_t *t)
{
	t->luma_log2_weight_denom = 0;
	t->delta_chroma_log2_weight_denom = 0;

	t->luma_weight_l0_flag.clear();
	t->chroma_weight_l0_flag.clear();
	t->delta_chroma_offset_l0.clear();
	t->luma_offset_l0.clear();
	t->delta_chroma_weight_l0.clear();
	t->delta_luma_weight_l0.clear();

	t->luma_weight_l1_flag.clear();
	t->chroma_weight_l1_flag.clear();
	t->delta_chroma_offset_l1.clear();
	t->luma_offset_l1.clear();
	t->delta_chroma_weight_l1.clear();
	t->delta_luma_weight_l1.clear();
}
...
inline void init_ref_pic_set_t(st_ref_pic_set_t *t)
{
	PART_INIT(t, inter_ref_pic_set_prediction_flag, abs_delta_rps_minus1);
	t->used_by_curr_pic_flag.clear();
	t->use_delta_flag.clear();
	PART_INIT(t, num_negative_pics, used_by_curr_pic_s1_flag);
}
...
inline void init_refPPS_t(referencePictureSets_t *t)
{
	memset(t, 0, sizeof(referencePictureSets_t));
}
...
inline void init_vps_t(h265_vps_t *t)
{
	PART_INIT(t, vps_video_parameter_set_id, vps_num_layer_sets_minus1);
	t->layer_id_included_flag.clear();
	PART_INIT(t, vps_timing_info_present_flag, vps_num_hrd_parameters);
	t->hrd_layer_set_idx.clear();
	t->cprms_present_flag.clear();
	init_hrd_t(&t->hrd_parameters);
	t->vps_extension_flag = 0;
	t->vps_extension_data_flag = 0;
}
...
inline void init_sps_t(h265_sps_t *t)
{
	t->sps_video_parameter_set_id =
		t->sps_max_sub_layers_minus1 =
		t->sps_temporal_id_nesting_flag = 0;

	init_ptl_t(&t->ptl);
	
	PART_INIT(t, sps_seq_parameter_set_id, num_short_term_ref_pic_sets);
	t->st_ref_pic_set.clear();
	t->m_RPSList.clear();
	PART_INIT(t, long_term_ref_pics_present_flag, lt_ref_pic_poc_lsb_sps_bytes);
	t->lt_ref_pic_poc_lsb_sps.clear();
	t->used_by_curr_pic_lt_sps_flag.clear();
	PART_INIT(t, sps_temporal_mvp_enabled_flag, vui_parameters_present_flag);
	init_vui_t(&t->vui);
	PART_INIT(t, sps_extension_present_flag, inter_view_mv_vert_constraint_flag);
}
...
inline void init_pps_t(h265_pps_t *t)
{
	PART_INIT(t, pps_pic_parameter_set_id, uniform_spacing_flag);
	t->column_width_minus1.clear();
	t->row_height_minus1.clear();
	PART_INIT(t, loop_filter_across_tiles_enabled_flag, pps_extension_5bits);
	init_pps_ex_t(&t->pps_range_extension);
	t->pps_range_extension_flag = 0;
}
...
typedef struct
{
	...
	uint8_t slice_reserved_flag[32];
	int slice_type;
	...
	int entry_point_offset_minus1[32];
	int slice_segment_header_extension_length;
	int slice_segment_header_extension_data_byte[32];
	// byte_alignment( )...
} h265_slice_header_t;

inline void init_slice_header_t(h265_slice_header_t *t)
{
	PART_INIT(t, read_slice_type, short_term_ref_pic_set_sps_flag);

	init_ref_pic_set_t(&t->st_ref_pic_set);

	PART_INIT(t, m_pRPS, collocated_ref_idx);

	init_pred_weight_table_t(&t->pred_weight_table);

	PART_INIT(t, five_minus_max_num_merge_cand, slice_segment_header_extension_data_byte);
}
