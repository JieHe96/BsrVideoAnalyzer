...
void  h265_read_vps_rbsp(h265_stream_t* h, bs_t* b)
{
	...
	for (i = 1; i <= vps->vps_num_layer_sets_minus1; i++)
	{
		...
		{
			a.push_back(bs_read_u1(b));
		}
		vps->layer_id_included_flag.push_back(a);
	}
#else
...
void  h265_read_sps_rbsp(h265_stream_t* h, bs_t* b)
{
	...
	profile_tier_level_t *profile_tier_level = new profile_tier_level_t();
...
void h265_read_slice_layer_rbsp(h265_stream_t* h, bs_t* b)
{
	...
	h265_slice_data_rbsp_t* slice_data = h->slice_data;
...
void h265_read_sei_rbsp(h265_stream_t* h, bs_t* b)
{
	...
	do {
		h->num_seis++;
		h->seis = (h265_sei_t**)realloc(h->seis, h->num_seis * sizeof(h265_sei_t*));
